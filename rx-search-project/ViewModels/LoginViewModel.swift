...
extension LoginViewModel{
    ...
    func createUser(_ githubId: String, _ email: String, _ password: String, completionBlock: @escaping (_ success: Bool) -> Void){
        loginManager.createUser(githubId: githubId, email: email, password: password) {success in
            if success {
                completionBlock(true)
            }else{
                completionBlock(false)
